...
public class DataBase {
    ...
    public List<Integer> obterTodasConfiguracoes(Connection conexao) throws SQLException {
        List<Integer> configuracoes = new ArrayList<>();
        Servidor servidor = new Servidor();

        String sql = "SELECT idConfiguracao FROM configuracao";
    ...
    public void inserirDados(Connection conexao, Double dadosCapturados, Integer fkTipoDado) throws SQLException {
        Date dataHoraAtual = new Date();
        Timestamp timestamp = new Timestamp(dataHoraAtual.getTime());
        List<Integer> configuracoes = obterTodasConfiguracoes(conexao);
    ...
    public void inserirDados(Connection conexao, Long dadosCapturados, Integer fkTipoDado) throws SQLException {
        Date dataHoraAtual = new Date();
        Timestamp timestamp = new Timestamp(dataHoraAtual.getTime());
        List<Integer> configuracoes = obterTodasConfiguracoes(conexao);
        ...
        for (Integer fkConfiguracao : configuracoes) {
            ...
            try (PreparedStatement ps = conexao.prepareStatement(insercao)) {
                ...
                ps.setInt(3, fkConfiguracao);
                ps.setInt(4, fkTipoDado);
                System.out.println("Dado Capturado: " + dadosCapturados);
